...
def load_bendr_model():
    ...
    Requirements
    ────────────
    pip install braindecode torch huggingface_hub
...
def _get_bendr_embeddings(
    ...
    with torch.no_grad():
        for start in range(0, len(epochs), batch_size):
            ...
            # extract_features returns the encoder output before the head
            features = model.extract_features(batch)  # (B, D) or (B, T, D)
            if features.dim() == 3:
