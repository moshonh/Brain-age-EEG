...
# ══════════════════════════════════════════════════════════════════
# TRAINING DATA COLLECTION & MODEL TRAINING ENGINE
# ══════════════════════════════════════════════════════════════════

TRAINING_STORE_KEY = "training_records"   # session_state key

FEATURE_COLS = [
    "apf",            # Alpha Peak Frequency (Hz)
    "posterior_dar",  # Delta/Alpha ratio — posterior channels only
    "delta_post",     # Mean relative delta power (posterior)
    "theta_post",     # Mean relative theta power (posterior)
    "alpha_post",     # Mean relative alpha power (posterior)
    "beta_post",      # Mean relative beta power (posterior)
]


def extract_training_features(result: dict) -> dict:
    """
    Extract features for Ridge Regression training.

    Includes:
    • 6 band-power features  (always available)
    • 120 Riemannian features (if pyriemann installed + enough epochs)

    The riemann_feat is stored as a list (JSON-serialisable) so it can
    survive session restarts if the user exports and re-imports the JSON.

    Parameters
    ----------
    result : dict returned by preprocess_and_extract()

    Returns
    -------
    features : dict with band-power keys + "riemann_feat" (list of 120)
    """
    br = result["band_rel"]

    # ── Band-power features (6) ───────────────────────────────────
    feats = {
        "apf":           result["apf"],
        "posterior_dar": result["posterior_dar"],
        "delta_post":    float(br["Delta"][DAR_IDX].mean()),
        "theta_post":    float(br["Theta"][DAR_IDX].mean()),
        "alpha_post":    float(br["Alpha"][DAR_IDX].mean()),
        "beta_post":     float(br["Beta"][DAR_IDX].mean()),
    }

    # ── Riemannian tangent-space features (120) ───────────────────
    riemann = extract_riemannian_features(result)
    feats["riemann_feat"] = riemann.tolist()   # list for JSON compat

    return feats


def extract_riemannian_features(result: dict) -> np.ndarray:
    """
    Compute Riemannian tangent-space features from EEG epoch covariances.

    Method (Engemann et al. 2022 — best-performing EEG brain age pipeline):
    ─────────────────────────────────────────────────────────────────────
    1. Estimate regularised covariance matrix per epoch (OAS estimator)
       on the 15 posterior + central channels (artifact-free)
    2. Project all covariance matrices to the tangent space of their
       Riemannian mean → symmetric positive-definite (SPD) manifold
    3. Vectorise upper triangle → 15×16/2 = 120 features per recording

    These features capture the full spatial correlation structure of the
    EEG across channels, not just per-band power — giving ~2 yr MAE
    improvement over band powers alone (Engemann et al. 2022).

    Parameters
    ----------
    result : dict from preprocess_and_extract()

    Returns
    -------
    features : (120,) float64 array — tangent-space vector
               Returns zeros if pyriemann is not installed.
    """
    try:
    ...
    epochs_post = result.get("epochs_posterior")
    if epochs_post is None or len(epochs_post) < 2:
        return np.zeros(120, dtype=np.float64)

    # Use float64 for numerical stability on SPD manifold
    X = epochs_post.astype(np.float64)

    try:
        cov  = Covariances(estimator="oas")
        C    = cov.fit_transform(X)          # (N, 15, 15)
        ts   = TangentSpace(metric="riemann")
        feat = ts.fit_transform(C)           # (N, 120)
        return feat.mean(axis=0)             # (120,) — mean over epochs
    except Exception:
        return np.zeros(120, dtype=np.float64)


# ══════════════════════════════════════════════════════════════════
# GOOGLE SHEETS BACKEND — Persistent shared storage
# ══════════════════════════════════════════════════════════════════
#
# All training records are stored in a Google Sheet, shared across
# ALL users of the same Streamlit deployment.
#
# Setup (one-time):
# ─────────────────
...
# 4. Create a Google Sheet with headers (row 1):
#    filename | chrono_age | apf | posterior_dar | delta_post |
#    theta_post | alpha_post | beta_post | riemann_feat
...
#
# Local dev: place the same secrets in .streamlit/secrets.toml
...
GSHEET_HEADERS = [
    "filename", "chrono_age",
    "apf", "posterior_dar",
    "delta_post", "theta_post", "alpha_post", "beta_post",
    "riemann_feat",
]
...
def _get_gsheet_worksheet():
    """
    Return a gspread Worksheet object connected to the training data sheet.
    Returns None if Google Sheets is not configured (local dev without secrets).
    """
    try:
        import gspread
        ...
        return None   # Graceful fallback — local dev / not configured


def _sheet_available() -> bool:
    """True when Google Sheets is configured and reachable."""
    return _get_gsheet_worksheet() is not None


def _record_to_row(record: dict) -> list:
    """Serialise a training record to a flat list matching GSHEET_HEADERS."""
    riemann = record.get("riemann_feat", [])
    return [
        record.get("filename",      ""),
        record.get("chrono_age",    ""),
        record.get("apf",           ""),
        record.get("posterior_dar", ""),
        record.get("delta_post",    ""),
        record.get("theta_post",    ""),
        record.get("alpha_post",    ""),
        record.get("beta_post",     ""),
        _json.dumps(riemann) if riemann else "[]",
    ]


def _row_to_record(row: list) -> dict | None:
    """Deserialise a sheet row back to a feature dict. Returns None on error."""
    try:
        if len(row) < 8:
            return None
        riemann_raw = row[8] if len(row) > 8 else "[]"
        return {
            "filename":      row[0],
            "chrono_age":    float(row[1]),
            "apf":           float(row[2]),
            "posterior_dar": float(row[3]),
            "delta_post":    float(row[4]),
            "theta_post":    float(row[5]),
            "alpha_post":    float(row[6]),
            "beta_post":     float(row[7]),
            "riemann_feat":  _json.loads(riemann_raw) if riemann_raw else [],
        }
    except Exception:
        return None


def _load_persistent_records() -> list[dict]:
    """
    Load all training records.

    Priority:
    1. session_state cache (avoids re-fetching on every Streamlit rerun)
    2. Google Sheet (source of truth — shared across users)
    3. Empty list (fallback when Sheets not configured)
    """
    # Use cached version within the same session to avoid hammering the API
    if TRAINING_STORE_KEY in st.session_state:
        return st.session_state[TRAINING_STORE_KEY]
    ...
    if ws is not None:
        try:
            ...
            records = [r for r in (_row_to_record(row) for row in rows[1:])
                       if r is not None]
            st.session_state[TRAINING_STORE_KEY] = records
            return records
        except Exception:
            pass

    st.session_state[TRAINING_STORE_KEY] = []
    return []


def _save_persistent_records(records: list[dict]) -> None:
    """
    Update session_state and rewrite Google Sheet with the full record list.
    """
    st.session_state[TRAINING_STORE_KEY] = records
    ...
    try:
        ...
        st.warning(f"⚠️ Google Sheets write failed: {exc}")


def _append_one_record_to_sheet(record: dict) -> None:
    """
    Append a single record to the sheet — faster than rewriting everything.
    Used by add_training_record() for efficiency.
    """
    ws = _get_gsheet_worksheet()
    if ws is None:
        return
    try:
        ws.append_row(_record_to_row(record), value_input_option="RAW")
    except Exception as exc:
        st.warning(f"⚠️ Google Sheets append failed: {exc}")


def add_training_record(filename: str, chrono_age: float, features: dict) -> None:
    """
    Append one record to the persistent training store (Google Sheets).

    - Appends a single row to the Sheet (fast, no full rewrite)
    - Also updates session_state cache immediately
    - Falls back to session_state-only when Sheet not configured
    """
    records = _load_persistent_records()
    record  = {"filename": filename, "chrono_age": chrono_age}
    record.update(features)

    # Avoid exact duplicates (same filename + age)
    if not any(r["filename"] == filename and r["chrono_age"] == chrono_age
               for r in records):
        records.append(record)
        st.session_state[TRAINING_STORE_KEY] = records   # update cache
        _append_one_record_to_sheet(record)               # fast sheet append


def get_training_records() -> list[dict]:
    """
    Return all training records from Google Sheets (shared across users).
    Falls back to session_state when Sheets not configured.
    """
    return _load_persistent_records()


def records_to_arrays(records: list[dict]) -> tuple[np.ndarray, np.ndarray]:
    """Convert record list to (X, y) numpy arrays for sklearn."""
    X = np.array([[r[c] for c in FEATURE_COLS] for r in records], dtype=float)
    y = np.array([r["chrono_age"] for r in records], dtype=float)
    return X, y
...
def train_ridge_model(
    ...
    """
    Train a Ridge Regression model on collected training records.

    Two modes (Engemann et al. 2022):
    ──────────────────────────────────
    Standard  (use_riemann=False):
        Features: 6 band-power features (APF, DAR, δ%, θ%, α%, β%)
        Requires: scikit-learn only
        MAE:      ~8–12 yrs

    Riemannian (use_riemann=True):
        Features: 120 tangent-space + 6 band-power = 126 features
        Requires: pyriemann  (`pip install pyriemann`)
        MAE:      ~6–9 yrs  (+2 yr improvement vs Standard)
        Reference: Engemann et al. NeuroImage 2022

    Returns
    -------
    model     : fitted sklearn Pipeline
    cv_mae    : cross-validated MAE in years
    r2        : R² on training data
    mode_used : "riemannian" or "standard"
    """
    ...
    # ── Feature matrix ────────────────────────────────────────────
    X_band, _ = records_to_arrays(records)          # (N, 6)
    mode_used  = "standard"

    if use_riemann:
        riemann_feats = np.array(
            [r.get("riemann_feat", np.zeros(120)) for r in records],
            dtype=np.float64,
        )                                            # (N, 120)
        has_riemann = np.any(riemann_feats != 0, axis=1).all()

        if has_riemann:
            X = np.hstack([riemann_feats, X_band])  # (N, 126)
            mode_used = "riemannian"
        else:
            X = X_band
            st.warning(
                "⚠️ Riemannian features not available in stored records. "
                "Re-save recordings to include Riemannian features, "
    ...
    return pipe, cv_mae, r2, mode_used


def records_to_csv_bytes(records: list[dict]) -> bytes:
    """Serialise training records to CSV bytes for download."""
    import csv, io as _io
    buf = _io.StringIO()
    cols = ["filename", "chrono_age"] + FEATURE_COLS
...
def records_to_json_bytes(records: list[dict]) -> bytes:
    import json
    return json.dumps(records, indent=2, ensure_ascii=False).encode()


def save_model_bytes(model) -> bytes:
    """Serialise fitted sklearn model to bytes via joblib."""
...
def preprocess_and_extract(edf_bytes: bytes, filename: str) -> dict:
    ...
    return {
        ...
        "epochs_posterior": good[:, DAR_IDX, :],  # (N_good, 15, 1000) posterior only
    }
    ...
    if ext_model is not None:
        try:
            ...
            band_feat = np.array([[
                result["apf"],
                pdar,
                float(br["Delta"][DAR_IDX].mean()),
                float(br["Theta"][DAR_IDX].mean()),
                float(br["Alpha"][DAR_IDX].mean()),
                float(br["Beta"][DAR_IDX].mean()),
            ]])                                         # (1, 6)

            # Try Riemannian (126 features) first, fallback to band (6)
            try:
                riemann = extract_riemannian_features(result)  # (120,)
                feat_riemann = np.hstack([riemann[np.newaxis, :], band_feat])
                pred = float(ext_model.predict(feat_riemann)[0])
            except Exception:
                pred = float(ext_model.predict(band_feat)[0])
...
def main():
    ...
    # ══════════════════════════════
    # ── Training Data Collection bar ────────────────────────────
    if chrono_age is not None:
        n_records = len(get_training_records())
        col_save, col_status = st.columns([1, 3])
        with col_save:
            if st.button(
                "💾 Save to Training Data",
                use_container_width=True,
                help="Save this recording's features + chronological age for model training",
            ):
                feats = extract_training_features(result)
                add_training_record(edf_file.name, chrono_age, feats)
                st.success(
                    f"✅ Saved! Training set now has "
                    f"{len(get_training_records())} recording(s)."
                )
        with col_status:
            n = len(get_training_records())
            if n == 0:
                msg = "No training data yet — upload recordings with known age and click 💾"
                col = "#8b949e"
            elif n < 10:
                msg = f"📊 {n} recording(s) saved · Need ≥10 for reliable training"
                col = "#d29922"
            elif n < 30:
                msg = f"📊 {n} recording(s) · Good — train now or collect more for better accuracy"
                col = "#3fb950"
            else:
                msg = f"📊 {n} recording(s) · Excellent dataset — ready to train"
                col = "#58a6ff"
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:0.75rem;'
                f'color:{col};padding-top:0.6rem;">{msg}</div>',
                unsafe_allow_html=True,
            )
    ...
    st.markdown('<hr class="section-rule">', unsafe_allow_html=True)
    with st.expander(
        f"🧠 Training Data & Model Builder  "
        f"({'%d recording(s)' % len(get_training_records()) if get_training_records() else 'no data yet'})",
        expanded=len(get_training_records()) >= 10,
    ):
        records = get_training_records()

        # Google Sheets connection status
        sheet_ok = _sheet_available()
        n_stored = len(records)
        if sheet_ok:
            sheet_status = (
                f'<span style="color:#3fb950;">🟢 Google Sheets connected</span> · '
                f'<b style="color:#e6edf3;">{n_stored} recording(s)</b> '
                f'shared across all users'
            )
        else:
            sheet_status = (
                f'<span style="color:#d29922;">🟡 Local session only</span> · '
                f'<b style="color:#e6edf3;">{n_stored} recording(s)</b> · '
                f'Configure Google Sheets in Secrets to share across users'
            )
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;'
            f'color:#8b949e;margin-bottom:0.8rem;">{sheet_status}</div>',
            unsafe_allow_html=True,
        )
        ...
        if not records:
            st.markdown(
                ...
                '<b style="color:#e6edf3;">How to build your own Brain Age model:</b><br><br>'
                '1. Upload an EDF recording<br>'
                '2. Enter the patient chronological age in the sidebar<br>'
                '3. Click <b style="color:#3fb950;">💾 Save to Training Data</b><br>'
                '4. Repeat for ≥10 recordings (30–50 recommended)<br>'
                '5. Click <b style="color:#58a6ff;">🧠 Train Model</b> to get a .joblib file<br>'
                '6. Load the .joblib via the sidebar → Custom model<br><br>'
                '<b style="color:#d29922;">Features used:</b> APF · Posterior DAR · '
                'δ% · θ% · α% · β% (posterior channels only)'
                '</div>',
                unsafe_allow_html=True,
            )
        else:
            ...
            # ── Stats bar ─────────────────────────────────────────
            ages = [r["chrono_age"] for r in records]
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Recordings",   len(records))
            c2.metric("Age range",    f"{min(ages):.0f} – {max(ages):.0f} yrs")
            c3.metric("Mean age",     f"{np.mean(ages):.1f} yrs")
            c4.metric("Std age",      f"{np.std(ages):.1f} yrs")

            # ── Training records table ─────────────────────────────
            df = pd.DataFrame(records)
            df_display = df[["filename", "chrono_age", "apf", "posterior_dar",
                              "delta_post", "alpha_post"]].copy()
            df_display.columns = ["File", "Age", "APF (Hz)", "Post. DAR",
                                   "Δ%", "α%"]
            df_display["Δ%"] = (df_display["Δ%"] * 100).round(1)
            df_display["α%"] = (df_display["α%"] * 100).round(1)
            df_display["APF (Hz)"] = df_display["APF (Hz)"].round(2)
            df_display["Post. DAR"] = df_display["Post. DAR"].round(3)
            st.dataframe(df_display, use_container_width=True, hide_index=True)

            # ── Download training data ─────────────────────────────
            st.markdown("**Download training data:**")
            dl1, dl2, dl3 = st.columns(3)
            with dl1:
                st.download_button(
                    "⬇️ Download CSV",
                    data     = records_to_csv_bytes(records),
                    file_name= "brain_age_training_data.csv",
                    mime     = "text/csv",
                    use_container_width=True,
                )
            with dl2:
                st.download_button(
                    "⬇️ Download JSON",
                    data     = records_to_json_bytes(records),
                    file_name= "brain_age_training_data.json",
            ...
            with dl3:
                ...
                with col_ref:
                    if st.button("🔄 Refresh",
                                 use_container_width=True,
                                 help="Reload records from Google Sheets"):
                        # Clear session cache → force reload from Sheet
                        if TRAINING_STORE_KEY in st.session_state:
                            del st.session_state[TRAINING_STORE_KEY]
                        st.rerun()
                with col_clr:
                    if st.button("🗑️ Clear all",
                                 use_container_width=True,
                                 type="secondary"):
                        _save_persistent_records([])
                        st.rerun()

            st.markdown("---")

            # ── Train model ────────────────────────────────────────
            st.markdown("**Train Ridge Regression model:**")

            if len(records) < 5:
                st.warning(f"Need at least 5 recordings to train. Have {len(records)}.")
            else:
                age_range = max(ages) - min(ages)
                if age_range < 15:
                    st.warning(
                        f"⚠️ Age range is only {age_range:.0f} years. "
                        "Add recordings across a wider age range for better accuracy."
                    )

                # ── Mode selector ────────────────────────────────
                riemann_available = any(
                    r.get("riemann_feat") and any(v != 0 for v in r["riemann_feat"])
                    for r in records
                )
                use_riemann = st.toggle(
                    "🔬 Riemannian mode (Engemann et al. 2022)",
                    value=riemann_available,
                    help=(
                        "Uses 120 tangent-space covariance features + 6 band-power "
                        "features. Requires pyriemann and gives ~2 yr MAE improvement "
                        "over standard band-power Ridge. Only available if recordings "
                        "were saved with Riemannian features (requires pyriemann installed)."
                    ),
                    disabled=not riemann_available,
                )
                if not riemann_available and use_riemann:
                    st.caption(
                        "Install pyriemann (`pip install pyriemann`) and "
                        "re-save recordings to enable Riemannian mode."
                    )

                col_train, col_info = st.columns([1, 2])
                with col_train:
                    mode_label = "🔬 Riemannian" if (use_riemann and riemann_available) else "📊 Standard"
                    if st.button(
                        f"🧠 Train Model ({mode_label})",
                        use_container_width=True,
                        type="primary",
                    ):
                        with st.spinner("Training Ridge Regression …"):
                            try:
                                model_trained, cv_mae, r2, mode_used = train_ridge_model(
                                    records,
                                    use_riemann=(use_riemann and riemann_available),
                                )
                                st.session_state["trained_model"]      = model_trained
                                st.session_state["trained_model_mae"]  = cv_mae
                                st.session_state["trained_model_r2"]   = r2
                                st.session_state["trained_model_mode"] = mode_used
                                st.success(
                                    f"✅ {mode_used.capitalize()} model trained! "
                                    f"CV-MAE = {cv_mae:.1f} yrs · R² = {r2:.3f}"
                                )
                            except Exception as exc:
                ...
                with col_info:
                    if "trained_model" in st.session_state:
                        mae  = st.session_state["trained_model_mae"]
                        r2   = st.session_state["trained_model_r2"]
                        mode = st.session_state.get("trained_model_mode", "standard")
                        quality = (
                            "🟢 Excellent" if (not np.isnan(mae) and mae < 5) else
                            "🟡 Good"      if (not np.isnan(mae) and mae < 10) else
                            "🔴 Needs more data"
                        )
                        mode_icon = "🔬" if mode == "riemannian" else "📊"
                        st.markdown(
                            f'<div style="background:#161b22;border:1px solid #30363d;'
                            f'border-radius:8px;padding:0.8rem 1rem;font-family:JetBrains Mono,'
                            f'monospace;font-size:0.78rem;color:#e6edf3;">'
                            f'<b>Last trained model</b><br>'
                            f'{mode_icon} Mode: {mode}<br>'
                            f'n = {len(records)} · R² = {r2:.3f}<br>'
                            f'CV-MAE = {"%.1f yrs" % mae if not np.isnan(mae) else "N/A"}<br>'
                            f'Quality: {quality}'
                            f'</div>',
                            unsafe_allow_html=True,
                        )
            ...
            if "trained_model" in st.session_state:
                st.markdown("**Download trained model (.joblib):**")
                col_dl, col_note = st.columns([1, 2])
                with col_dl:
                    st.download_button(
                        "⬇️ Download brain_age_model.joblib",
                        data      = save_model_bytes(st.session_state["trained_model"]),
                        file_name = "brain_age_model.joblib",
                ...
                with col_note:
                    st.markdown(
                        ...
                        'font-size:0.72rem;color:#8b949e;padding-top:0.3rem;">'
                        'Load via sidebar → Custom .joblib / .pkl<br>'
                        '<b style="color:#3fb950;">Standard:</b> 6 features<br>'
                        '<b style="color:#58a6ff;">Riemannian:</b> 126 features<br>'
                        'pip install pyriemann for Riemannian mode'
                        '</div>',
                        unsafe_allow_html=True,
                    )
