...
def _get_bendr_embeddings(
    ...
    with torch.no_grad():
        for start in range(0, len(epochs), batch_size):
            ...
            # Pretrained BENDR expects 20 channels — pad with a silent
            # zero channel so weights load correctly.
            if batch.shape[1] == 19:
...
def predict_age(
    ...
    Model priority (highest → lowest):
      1. Calibrated Linear Model  – if calibration data was provided
      2. BENDR Foundation Model   – if loaded from HuggingFace
      3. Uploaded .joblib / .pkl  – if supplied in the sidebar
      4. Built-in Reference Model – APF + posterior δ/α (always available)

    All paths now use posterior_dar (artifact-free) instead of whole-head dar.
    """
    pdar = result.get("posterior_dar", result["dar"])

    # ── 1. Calibrated model (highest priority) ────────────────────
    if cal_model is not None and cal_model.get("n", 0) >= 2:
        age = predict_calibrated(pdar, cal_model)
        return age, f"📐 Calibrated Model (n={cal_model['n']}, R²={cal_model['r2']:.2f})"
    ...
    # ── 4. Reference model (posterior DAR) ───────────────────────
    age = predict_reference_model(result["apf"], pdar)
    return age, "Built-in Reference Model (APF + posterior δ/α)"
