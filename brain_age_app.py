...
def add_training_record(filename: str, chrono_age: float, features: dict) -> None:
    """Append one record to the in-session training store."""
    if TRAINING_STORE_KEY not in st.session_state:
    ...
    existing = st.session_state[TRAINING_STORE_KEY]
...
def get_training_records() -> list[dict]:
    return st.session_state.get(TRAINING_STORE_KEY, [])
