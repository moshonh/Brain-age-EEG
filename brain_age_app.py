...
def preprocess_and_extract(edf_bytes: bytes, filename: str) -> dict:
    ...
    def _band_power(f, Pxx, lo, hi):
        ...
        return np.trapz(Pxx[:, idx], f[idx], axis=1)   # (channels,)
...
def predict_age(result: dict, ext_model=None) -> Tuple[float, str]:
    """
    Return (predicted_age, model_label).
    Tries external model first; falls back to reference model.
    """
    if ext_model is not None:
        try:
            # Feature vector: [APF, DAR, mean relative band powers (4)]
            feat = np.array([[
                result["apf"],
                result["dar"],
            ...
            pred = np.clip(pred, 20.0, 80.0)
            return pred, "Loaded .joblib / .pkl model"
        except Exception as e:
            st.warning(f"External model failed ({e}). Falling back to reference model.")
...
def main():
    ...
    st.markdown(
        ...
        'RIDGE REGRESSION · ALPHA-PEAK + Δ/α BIOMARKERS</div>',
        unsafe_allow_html=True,
    )
    ...
    # ── Load external model (if supplied) ─────────────────────────
    ext_model = None
    ...
    # ── Prediction ────────────────────────────────────────────────
    brain_age, model_label = predict_age(result, ext_model)
