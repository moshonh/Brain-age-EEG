...
# El Ouahidi et al., NeurIPS 2025 · huggingface.co/brain-bzh/reve-base
#
# Architecture: Transformer encoder, 4D positional encoding, 69M params.
# Pretrained on 60,000 hrs of EEG from 92 datasets / 25,000 subjects.
# Accepts any electrode layout; we pass the standard 10-20 names.
# REVE requires 200 Hz input; we resample from our 250 Hz pipeline.

REVE_FS       = 200
REVE_REPO     = "brain-bzh/reve-base"
REVE_POS_REPO = "brain-bzh/reve-positions"


@st.cache_resource(show_spinner=False)
def load_reve_model():
    """
    Download REVE encoder + position bank from HuggingFace Hub.

    Returns (encoder, pos_bank) tuple, both in eval() mode.

    Requirements
    ------------
    pip install transformers torch huggingface_hub

    Note: brain-bzh/reve-base is a gated model — you must be logged in
    with `huggingface-cli login` and have accepted the model's terms on
    the Hub before the first download.
    """
    from transformers import AutoModel
    pos_bank = AutoModel.from_pretrained(
        REVE_POS_REPO, trust_remote_code=True
    )
    encoder = AutoModel.from_pretrained(
        REVE_REPO, trust_remote_code=True
    )
    encoder.eval()
    pos_bank.eval()
    return encoder, pos_bank


def _resample_epochs_for_reve(epochs_250: np.ndarray) -> np.ndarray:
    """
    Downsample EEG epochs from 250 Hz → 200 Hz.

    Parameters
    ----------
    epochs_250 : (N, 19, 1000)  float64  at 250 Hz

    Returns
    -------
    epochs_200 : (N, 19, 800)   float32  at 200 Hz
    """
    from scipy.signal import resample
    n_out = int(epochs_250.shape[2] * REVE_FS / TARGET_FS)   # 800
    return resample(epochs_250, n_out, axis=2).astype(np.float32)


def _get_reve_embeddings(
    encoder,
    pos_bank,
    epochs: np.ndarray,
    batch_size: int = 8,
    ...
    """
    Run the REVE encoder in mini-batches over all clean epochs and
    return the **mean** embedding across epochs (one vector per recording).

    Parameters
    ----------
    encoder   : REVE AutoModel in eval mode
    pos_bank  : REVE position-bank AutoModel
    epochs    : (N, 19, 800) float32  at 200 Hz
    batch_size: number of epochs per GPU/CPU call

    Returns
    -------
    embedding : (embed_dim,) float32
    """
    import torch

    device    = next(encoder.parameters()).device
    positions = pos_bank(STANDARD_19)          # (19, 3) – 3-D electrode coords
    all_embs  = []

    with torch.no_grad():
        for start in range(0, len(epochs), batch_size):
            batch = torch.from_numpy(
                epochs[start : start + batch_size]
            ).to(device)                        # (B, 19, 800)

            # Expand position tensor to match batch dimension
            pos = positions.expand(batch.size(0), -1, -1)   # (B, 19, 3)
            out = encoder(batch, pos)

            # REVE returns BaseModelOutput; average over the token / patch axis
    ...
    # Average embeddings across all accepted epochs → single vector
    return np.concatenate(all_embs, axis=0).mean(axis=0)   # (D,)


def _reve_linear_head(embedding: np.ndarray, apf: float, dar: float) -> float:
    """
    Linear regression head on top of REVE embeddings.

    Production use: fit sklearn Ridge on (embedding, age) pairs from
    a normative dataset and save with joblib.  Load via the sidebar
    .joblib uploader, then wire predict_with_reve() to use those weights.

    Out-of-the-box (no labelled data): use a fixed random projection
    seeded at 42, calibrated to the physiological reference model.
    This ensures the output is anatomically plausible and centred on
    the reference-model estimate with a ±5-yr neural modulation.
    """
    ...
    # Scalar projection; tanh keeps it bounded ∈ (−1, +1)
    proj    = float(np.dot(w_rnd, embedding))
    ref_age = predict_reference_model(apf, dar)

    # 5-yr modulation around the reference model's estimate
    blended = ref_age + 5.0 * float(np.tanh(proj))
    return float(np.clip(blended, 20.0, 80.0))


def predict_with_reve(encoder, pos_bank, result: dict) -> Tuple[float, str]:
    """
    Full REVE prediction pipeline:
      raw epochs (250 Hz)
        → resample 200 Hz
        → REVE encoder (mini-batched)
        → mean embedding
        → linear regression head
        → predicted age (years)
    """
    epochs_200 = _resample_epochs_for_reve(result["epochs_clean"])
    embedding  = _get_reve_embeddings(encoder, pos_bank, epochs_200)
    age        = _reve_linear_head(embedding, result["apf"], result["dar"])
    return age, "🤗 REVE Foundation Model (brain-bzh/reve-base)"
...
def predict_age(
    result:      dict,
    ext_model=   None,
    reve_models: Optional[Tuple] = None,
) -> Tuple[float, str]:
    """
    Model priority (highest → lowest):
      1. REVE Foundation Model        – if loaded from HuggingFace
      2. Uploaded .joblib / .pkl       – if supplied in the sidebar
      3. Built-in Reference Model      – APF + δ/α biomarkers (always available)
    """
    # ── 1. REVE ──────────────────────────────────────────────────
    if reve_models is not None:
        try:
            return predict_with_reve(*reve_models, result)
        except Exception as exc:
            st.warning(f"REVE inference failed ({exc}). Falling back.")
...
def main():
    ...
    st.markdown(
        ...
        'RIDGE REGRESSION · ALPHA-PEAK + Δ/α · REVE FOUNDATION MODEL</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        model_choice = st.radio(
            "Backend",
            options=["Reference Model", "REVE (HuggingFace)", "Custom .joblib / .pkl"],
            index=0,
            help="Choose the prediction backend. REVE requires internet access and "
                 "a HuggingFace account with access to brain-bzh/reve-base.",
        ...
        elif model_choice == "REVE (HuggingFace)":
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;'
                'color:#8b949e;line-height:1.5;padding:0.5rem 0;">'
                '🤗 <b style="color:#e6edf3;">brain-bzh/reve-base</b><br>'
                'Transformer · 69M params<br>'
                'Pretrained on 60k hrs EEG<br>'
                '25,000 subjects · 92 datasets<br><br>'
                '⚠️ Requires HF login + model access.<br>'
                'Run: <code>huggingface-cli login</code>'
                '</div>',
                unsafe_allow_html=True,
            )
            if st.button("⬇️ Load REVE from HuggingFace", use_container_width=True):
                with st.spinner("Downloading REVE (~270 MB) …"):
                    try:
                        reve_models = load_reve_model()
                        st.session_state["reve_models"] = reve_models
                        st.success("✅ REVE loaded!")
                    except Exception as exc:
                        st.error(f"❌ {exc}")

            # Persist across reruns
            if "reve_models" in st.session_state:
                reve_models = st.session_state["reve_models"]
                st.success("✅ REVE ready")
        ...
        if model_choice == "REVE (HuggingFace)":
            st.markdown(
                '<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;'
                'color:#8b949e;line-height:1.6;">'
                '<b style="color:#58a6ff;">REVE pipeline</b><br>'
                'epochs (250 Hz)<br>'
                '→ resample 200 Hz<br>'
                '→ REVE encoder<br>'
                '→ mean embedding<br>'
                '→ linear head → age<br><br>'
                'Head calibrated to the<br>'
                'reference model.<br>'
                'Fine-tune on labelled<br>'
                'EEG for best accuracy.'
                '</div>',
                unsafe_allow_html=True,
            )
        else:
    ...
    # ── Prediction ────────────────────────────────────────────────
    brain_age, model_label = predict_age(result, ext_model, reve_models=reve_models)
